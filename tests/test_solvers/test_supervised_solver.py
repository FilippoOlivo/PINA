...
def test_train_cpu():
    ...
    trainer = Trainer(solver=solver, max_epochs=200, accelerator='cpu', batch_size=5, train_size=1, test_size=0., eval_size=0.)
    trainer.train()
test_train_cpu()
...
def test_extra_features_train_cpu():
    solver = SupervisedSolver(problem=problem, model=model_extra_feats, extra_features=extra_feats)
    trainer = Trainer(solver=solver, max_epochs=200, accelerator='cpu', batch_size=5)
    trainer.train()